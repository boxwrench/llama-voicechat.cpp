...
Status: this produces the weights and the hyper-parameters. The ggml graph that
consumes them is not written yet; see tools/voicechat/README.md.
"""
...
def main() -> None:
    ...
    def put_src(dst_name: str, src_name: str, dtype=np.float32, squeeze=None) -> None:
        a = src.f32(src_name)
    ...
    for il in range(n_layer):
        ...
        put_src(b + "attn_q_norm.weight", p + "self_attn.q_norm.weight")
        put_src(b + "attn_k_norm.weight", p + "self_attn.k_norm.weight")
        ...
        put_src(b + "attn_norm.weight",      p + "input_layernorm.weight")
        put_src(b + "post_attention_norm.weight", p + "post_attention_layernorm.weight")
        put_src(b + "ffn_norm.weight",       p + "pre_feedforward_layernorm.weight")
        put_src(b + "post_ffw_norm.weight",  p + "post_feedforward_layernorm.weight")

    put_src("output_norm.weight", TTS + "backbone.norm.weight")
    ...
    put_src(eb + "attn_norm.weight",           ep + "layers.0.pre_self_attn_layernorm.weight")
    put_src(eb + "post_attention_norm.weight", ep + "layers.0.post_self_attn_layernorm.weight")
    put_src(eb + "ffn_norm.weight",            ep + "layers.0.pre_feedforward_layernorm.weight")
    put_src(eb + "post_ffw_norm.weight",       ep + "layers.0.post_feedforward_layernorm.weight")
    put_src(sb + "output_norm.weight",         ep + "norm.weight")
    ...
    put_src(gb + "norm.weight",        gp + "final_norm.weight")
    ...
    for i in range(mog["num_layers"]):
        ...
        copy_raw(p + "mlp.up_proj.weight",   b + "ffn_up.weight")
        copy_raw(p + "mlp.down_proj.weight", b + "ffn_down.weight")
        put_src(b + "pre_norm.weight",  p + "pre_norm.weight")
        put_src(b + "post_norm.weight", p + "post_norm.weight")
    put_src("tts.mog.norm.weight", f"{TTS}mog_head.mlp_stack.{mog['num_layers']}.weight")
